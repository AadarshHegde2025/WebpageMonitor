...
def url_checker(url, row):
    ...
    if (url in url_to_html.keys()):
        old_html_code = str(url_to_html.get(url))
        ...
        if (old_html_code == true_new_html):
            ...
            while (i < get_len_rows()):
                if (i != get_len_rows() - 1 and not lines[i] == old_html_code):
                    new_file.write(lines[i])
                elif (not lines[i] == old_html_code and i == get_len_rows() - 1):
                    #Have to make it be placed in the same order
                    new_file.write(lines[i] + '\n')
                elif (lines[i] == old_html_code and i == get_len_rows() - 1):
                    new_file.write(true_new_html + '\n')
                else:
                    new_file.write(true_new_html)
                i+=1
            SendMail(url)
...
def Updater():
    ...
    while (i < len(row_to_url)):
        ...
        i+=1
    root.after(600000, Updater)   # the delay is in milliseconds
...
def DeleteWebpage():
    ...
    if (int(row_to_delete) > get_len_rows() or int(row_to_delete) < 0):
        print("Row Does Not Exist. Please Try Again")
        DeleteWebpage()
    else:
        # table._pop_n_rows(1)
        global table
        ...
        with open(website_mem_location, 'r') as file:
            data = file.readlines()
        data.pop(int(row_to_delete))
        with open(website_mem_location, 'w') as file:
            file.writelines( data )
        # Update HTML Memory
        with open(html_mem_location, 'r') as file:
            data2 = file.readlines()
        data2.pop(int(row_to_delete))
        with open(html_mem_location, 'w') as file:
...
def SendTest():
    ...
    <body>
        <p>Hi,<br>
        <br>
        This message was sent from Webpage Monitor App to inform as a test to ensure notification system is workingbr>
        Thank you. <br>
        </p>
    </body>
    </html>
...
#Add a function to add more tables

# theme = Button(root, text="Change Theme",command=change_theme)
# theme.pack()
